        ...
        if (version < 4 && !m_additionalValues.empty()) {
            ...
            const auto valuesString = [&]() -> string {
                if (m_additionalValues.size() == 1) {
                    return argsToString("value \"", m_additionalValues.front().toString(TagTextEncoding::Utf8), "\" is ignored.");
                }
                return argsToString("values ", DiagMessage::formatList(TagValue::toStrings(m_additionalValues)), " are ignored.");
            }();
            diag.emplace_back(
